...
public class RotationObject1 : MonoBehaviour {
	...
	public GameObject Tige_ObjetController;
	...
	public bool newfreerotation = false;
...
//		}
		...
		if (Physics.Raycast (ray, out hit) && playeable == false && ismoving == false) {
			...
			if (hit.transform.tag == "feuille") {
				freerotation = true;
				newfreerotation = true;
				ObjetController = hit.transform.gameObject;
				Tige_ObjetController = ObjetController.transform.parent.gameObject;
				bougeable = true;
			}
		}
	...
	void ControleLibre () {
		...
		if (playeable == true && newfreerotation == false) {
            IsHolding = true;
		...
		if (playeable == true && newfreerotation == true) {
			print ("marche");
			IsHolding = true;
			Tige_ObjetController.transform.Rotate (0, -pointer_x * SpeedFreeRotation, 0, Space.World);
			ObjetController.transform.Rotate (pointer_y * SpeedFreeRotation, 0, 0, Space.World);
